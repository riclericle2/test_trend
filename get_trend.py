...
def read_email_from_gmail(sc):
    ...
    try:
        ...
        # -------------------- 分割線 --------------------
        # 讀取資料庫中的email_id
        email_id_in_database = pd.read_sql("select * from nqtrade.email_id2",con = engine)
        ...
        if latest_email_id > email_id_in_database:
          ...
          email_content2.columns=['方向','日期','散户','中户240','中户1200']
          email_content2=email_content2[['日期','方向','散户','中户240','中户1200']]
          # 上傳更新的數據到Database
          print(email_content2)
          email_content2.to_sql('trend', con = engine, if_exists = 'append', chunksize = 1000, index=False)
          ...
          if latest_email_id > email_id_in_database:
            ...
            update_email_id.to_sql('email_id2', con = engine, if_exists = 'append', chunksize = 1000, index=False)
